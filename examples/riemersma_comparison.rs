...
use ditherlib::{
    DiffusionAlgorithm, DiffusionScan, ErrorDiffusion, Palette, Pipeline, Point, Polygon, Renderer,
    RiemersmaDither, Selection, read, write,
};
...
fn main() -> ExitCode {
    ...
    let (Some(input), Some(output), None) = (arguments.next(), arguments.next(), arguments.next())
        ...
        eprintln!("usage: riemersma_comparison INPUT OUTPUT.png");
